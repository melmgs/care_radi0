...
[historyOverlay,notesOverlay,submitOverlay,rotationOverlay,listeningOverlay].forEach(overlay=>{
  if(!overlay)return;
...
document.querySelectorAll(".share-trigger").forEach(trigger=>{
  const activate=()=>{
    const isCover=trigger.classList.contains("cover-wrap");
...
/* ============================================================
   HEARD HERE
   personal listening memory stored only in this browser
============================================================ */
...
function recordHeardTrack(track){
  ...
  const existingIndex=
    heard.findIndex(item=>item.key===key);
  ...
  /* Keep the browser memory finite. */
  heard=heard.slice(-300);

  saveHeardTracks(heard);
}


function clearHeardTracks(){
  try{
...
function terminalHelp(){
  terminalLine(`
    <div class="terminal-help">
      <span>listen</span><span>recently</span>
      <span>rotations</span><span>submit</span>
      <span>track</span><span>random</span>
      <span>heard</span><span>notes</span>
      <span>close</span>
    </div>
  `);
}

function terminalRandomTrack(){
  const source=historyTracksCache.length
    ?historyTracksCache
    :recentTracksCache;

  if(!source.length){
    terminalLine("nothing in memory yet","terminal-muted");
    return;
  }
...
function terminalTrack(){
  if(!currentTrack.title){
    terminalLine("waiting for the stream","terminal-muted");
    return;
  }

  const key=
    `${normalizeText(currentTrack.artist)}::${normalizeText(currentTrack.title)}`;

  const note=editorialData.track_notes?.[key]||null;

  terminalLine(`
    <strong>${escapeHTML(currentTrack.title)}</strong><br>
...
function renderNotes(){
  ...
  notesList.innerHTML=editorialData.notes
    .map(note=>`
...
function openNotes(){
  renderNotes();
  openOverlay(notesOverlay,terminalTrigger);
}



function terminalHeard(){
  const heard=loadHeardTracks();

  if(!heard.length){
    terminalLine(
      "u haven't heard anything here yet",
      "terminal-muted"
    );
    return;
  }
  ...
  terminalLine(`
    <strong>heard here</strong>

    <br>

    <span class="terminal-muted">
      u've heard ${heard.length} different track${heard.length===1?"":"s"} here
    </span>
...
function terminalNotes(){
  ...
  const latest=editorialData.notes[0];
  ...
  terminalLine(`
    ...
    ${
      editorialData.notes.length > 1
        ?`
          <br>
          <button
            class="terminal-notes-link"
            type="button"
            data-open-notes
          >
            older notes ↗
...
function runTerminalCommand(rawCommand){
  ...
  if(["recent","recently","history"].includes(command)){
    terminalClose();
    openHistory();
    return;
  }
